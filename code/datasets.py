...
def get_imgs(img_path, imsize, bbox=None,
	...
	# mask_name = '%s/segmentations/%s.png' % (data_dir, key)
	mask_path = img_path.replace("images","segmentations")[:-3] + 'png'
	mask = Image.open(mask_path).convert('L')

	width, height = img.size
	if bbox is not None:
		r = int(np.maximum(bbox[2], bbox[3]) * 0.75)
		# r = int(np.maximum(bbox[2], bbox[3]) * 1)
	...
	re_cmask = transforms.Scale(imsize[1])(cmask)
	re_cmask = re_cmask.point(lambda p : p > 0)
	
	retc.append(normalize(re_cimg))
	retm.append(transforms.ToTensor()(re_cmask))
	...
	return retf, retc, retm, warped_bbox
...
class Dataset(data.Dataset):
	...
	def prepair_test_pairs(self, index):
		...
		data_dir = self.data_dir
		img_name = '%s/images/%s.jpg' % (data_dir, key)
		_, imgs, masks, _ = get_imgs(img_name, self.imsize,
						bbox, self.transform, normalize=self.norm)

		return imgs, masks, key 
