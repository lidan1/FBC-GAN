...
def define_optimizers(netG, netD):
	optimizersD = []
	optimizersD.append(optim.Adam(netD.parameters(),
...
class FineGAN_trainer(object):
	...
	def prepare_data(self, data):
		fimgs, cimgs, cmasks, c_code, _, warped_bbox = data
		...
		return fimgs, real_vfimgs, real_vcimgs, real_vcmasks, vc_code, warped_bbox
	...
	def train_Dnet(self, idx, count):
		...
		netD, optD = self.netD, self.optimizersD
	...
	def train_Gnet(self, count):
		self.netG.zero_grad()
		# for myit in range(len(self.netD)): 
		self.netD.zero_grad()
		...
		outputs = self.netD(self.fake_imgs[0]) 	
	...
	def train(self):
		self.netG, self.netD, start_count = load_network(self.gpus)
		avg_param_G = copy_G_params(self.netG)

		self.optimizerG, self.optimizersD = \
			define_optimizers(self.netG, self.netD)
			...
			for step, data in enumerate(self.data_loader, 0):
				...
				if count % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
					...
					with torch.set_grad_enabled(False):
						self.fake_imgs, = \
							self.netG(noise) #self.netG(fixed_noise)
						save_img_results(self.imgs_tcpu, (self.fake_imgs), 0,
